...
class TaskProcessor:
    """Processes typed Task objects into prioritized, schedulable items."""
    ...
    def process_tasks(self, tasks: List[Task]) -> List[Task]:
        """
        Process typed Task objects into a prioritized list of Task objects.
        Returns: List[Task]
        """
        logger.info(f"Processing {len(tasks)} raw tasks")
        
        grouped_tasks, _ = self._group_parent_and_subtasks(tasks)
    ...
    def _calculate_priority(self, total_effort_hours, deadline_dt):
        # Simplified for brevity, logic remains same as previous
        import datetime
        now = datetime.datetime.now(datetime.timezone.utc)
        if not deadline_dt: return PriorityTier.T6, float('inf'), 0.0
        if deadline_dt.tzinfo is None: deadline_dt = deadline_dt.replace(tzinfo=datetime.timezone.utc)
    ...
    def _calculate_project_urgency(self, grouped_tasks: List[Task]) -> List[Task]:
        ...
        for task in grouped_tasks:
            ...
            if subtasks and calculated_deadline: task.deadline = calculated_deadline
    ...
    def _expand_tasks_by_priority(self, prioritized_projects: List[Task]) -> List[Task]:
        """
        Expand projects (Task objects with urgency metrics) into individual schedulable items 
        (Task objects) based on priority.
        """
